...
class MyHashSet
    ...
    def delete(el)
        isDeleted = @store.delete(el) {"No"}
        (isDeleted != "No") ? true : false
    end
    ...
    def union(set2)
        ...
        @store.merge(set2.store).keys.each {|k| newSet.insert(k)}
        newSet
    end
    
    def intersect(set2)
        newSet = MyHashSet.new
        set2.store.keys.each {|k| newSet.insert(k) if include?(k)}
        newSet
    end
    
    def minus(set2)
        newSet = MyHashSet.new
        @store.keys.select {|k| !self.intersect(set2).to_a.include?(k)}.each {|k| newSet.insert(k)}
        newSet
    end
end
