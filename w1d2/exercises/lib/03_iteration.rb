...
class Array
  def my_map(&prc)
    out = []
    my_each{|element| out << prc.call(element)}
    out
  end

  def my_select(&prc)
    out = []
    my_each{|element| out << element if prc.call(element)}
    out
  end

  def my_inject(&blk)
    out = self.first
    self[1..-1].my_each {|element| out = blk.call(out, element)}
    out
  end
end
